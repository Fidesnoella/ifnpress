        ...
        <div className={` border-gray-400 ${last ? "border-0" : "border-b"} hover:bg-[#e3e2e0] cursor-pointer`} onClick={handleClick}>
            <div className="grid grid-cols-2 gap-2 w-full relative overflow-hidden">
                ...
                <div className="flex flex-col gap-3">
                    <h2 className="mb-2 text-sm xss:text-base sm:text-lg lg:text-base font-bold tracking-tight text-gray-500">{title?.substring(0, 60)}...</h2>
                    <span className="text-gray-500 text-xs sm:text-sm absolute bottom-2">{date}</span>
                    <p className="hidden sm:block lg:hidden mb-2 text-base font-bold tracking-tight text-gray-400 dark:text-white">{text?.substring(0, 85)}...</p>
                </div>
            </div>
        </div>
    );
